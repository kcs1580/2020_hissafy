          ...
          <v-dialog v-model="dialog" max-width="500px">
            <template v-slot:activator="{ on }">
              <router-link to="studentManagement">
                <v-btn color="primary" dark class="mb-2 mx-2">취소</v-btn>
              </router-link>
              <v-btn color="primary" dark class="mb-2" @click="editSave">저장</v-btn>
              <v-btn color="primary" dark class="mb-2 mx-2" v-on="on">추가</v-btn>
            </template>
            <v-card>
              <v-card-title>
                  ...
                  <v-form ref="form" v-model="valid" lazy-validation>
                    <v-row>
                      <v-col cols="12" sm="6" md="4">
                        <v-text-field v-model="editedItem.student_id" label="학번" :rules="id_rules"></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="6" md="4">
                      ...
                      <v-col cols="12" sm="6" md="4">
                        <v-text-field v-model="editedItem.class_num" type="number" label="반"></v-text-field>
                      </v-col>
                      <v-col cols="12" sm="6" md="4">
...
export default {
  ...
  methods: {
    ...
    editSave() {
      ...
      http
        .post("student/registerstudent", this.students)
        .then(response => {
          this.students = response.data.list;
